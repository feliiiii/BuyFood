...
namespace BuyFood_Template.Controllers
{
    public class OrderController : Controller
    {
        ...
        {
            擺腹BuyFoodContext db = new 擺腹BuyFoodContext();
            var OrderIDs = db.TOrders.OrderByDescending(n=>n.COrderId).Select(n => n.COrderId).ToList();
        ...
        public JsonResult getOrderTime_Quick()
        {
            擺腹BuyFoodContext db = new 擺腹BuyFoodContext();
            var OrderIDs = db.TOrders.OrderByDescending(n => n.COrderId).Select(n => n.COrderId).ToList().Take(10);
            foreach (var ID in OrderIDs)
            {
                var getOrderTime = db.TOrders.Where(n => n.COrderId == ID).Select(n => n.COrderDate).FirstOrDefault();
                DateTime OrderTime = DateTime.ParseExact(getOrderTime, "yyyy/MM/dd HH:mm:ss", null);
